...
#   3. implement 完成后必须进入 integrate
#   4. integrate 完成后必须进入 verify
#   5. 只有任务组整体完成后才允许通知 human
#   6. write 任务允许空 resource_keys，且相同 resource_keys 也允许并发认领
#   7. resume 缺少快照或 schema_version 不匹配时拒绝恢复
#
# 用法:
...
section "Test 3: implement 完成后必须进入 integrate"
setup_runtime
task_id=$(publish_task "develop" "落代码实现" "$(build_contract "implement" "write" "exclusive" '["repo:kernel"]')")
TEST_INSTANCE="implementer"
cmd_claim "$task_id" >/dev/null
cmd_complete_task "$task_id" "代码已经完成，等待验收" >/dev/null
...
section "Test 4: integrate 完成后必须进入 verify"
setup_runtime
...
section "Test 5: 只有任务组整体完成后才通知 human"
setup_runtime
...
section "Test 6: write 任务允许空 resource_keys，且相同 resource_keys 也允许并发认领"
setup_runtime
...
section "Test 7: resume 快照缺失或 schema_version 不匹配时拒绝恢复"
