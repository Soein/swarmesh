...
### 0. 解析意图（基于 $ARGUMENTS 第一个 token）

根据用户输入的第一个 token dispatch 到对应 `discuss-vote.sh` 子命令。仅当第一个 token **完全等于**下列关键字之一时才 dispatch，否则视作问题文本走 ask 流程：

| 第一个 token | 行为 |
|---|---|
| 空 / 无参数 | 调 `list`，列历史投票；提示用户发问用 `/swarm-vote "<问题>"` |
| `list` | 调 `discuss-vote.sh list` |
| `report` | 调 `discuss-vote.sh report --id <第 2 token>` |
| `collect` | 调 `discuss-vote.sh collect --id <第 2 token>` |
| `next-round` | 调 `discuss-vote.sh next-round --id <第 2 token>`（多轮辩论下一轮） |
| `cancel` | 调 `discuss-vote.sh cancel --id <第 2 token>` |
| 其他任何文本 | 作为"问题"走下面 ask 流程 |

**示例**：
```bash
/swarm-vote                             # 列历史
/swarm-vote list                        # 列历史（显式）
/swarm-vote report vote-xxx-abc         # 看指定投票报告
/swarm-vote next-round vote-xxx-abc     # 推进多轮辩论
